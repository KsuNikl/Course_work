...
bool convert_xlsx_to_csv(const std::string& xlsx_path, const std::string& csv_path, std::string& error)
{
    try {
        ...
        auto ws = wb.worksheet(sheetNames.at(0));
        ...
        for (uint32_t r = dataStartRow; r <= maxRows; ++r) {
            ...
            for (int fi = 0; fi < 5; ++fi) {
                ...
                int colIdx = fieldIndex[fieldName];
