...
bool import_threats_csv(Db& db, const std::string& csv_path, long user_id, ImportStats& stats, std::string& error)
{
    error.clear();
    stats = {};
    try {
        ...
        pqxx::stream_to stream(
            tx,
            "staging_threats",
            std::vector<std::string>{"threat_code", "title", "description", "consequences", "source"}
        );
        std::string line;
